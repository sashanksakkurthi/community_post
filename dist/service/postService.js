...
const createPost = async (req, res) => {
    ...
    try {
        const post = await (0, postRepository_1.CreatePost)(userId, content);
...
const updatePost = async (req, res) => {
    ...
    try {
        const post = await (0, postRepository_1.UpdatePost)(hash, content);
        res.sendStatus(200);
    }
    catch (error) {
        res.sendStatus(400);
...
const deletePost = async (req, res) => {
    ...
    try {
        const post = (0, postRepository_1.DeletePost)(hash);
...
const getLatestPosts = async (req, res) => {
    try {
        const posts = await (0, postRepository_1.LatestPosts)();
...
const getHashSpecificPosts = async (req, res) => {
    ...
    if (!hash) {
        ...
        try {
            const posts = await (0, postRepository_1.PostByHash)(hash);
...
const getUserSpecificPosts = async (req, res) => {
    ...
    if (!userId) {
        ...
        try {
            const posts = await (0, postRepository_1.UserSpecificPosts)(userId);
            res.status(200).json({ post: posts });
        }
        catch (error) {
            res.status(400).json({ error: "User Posts not founded" });
