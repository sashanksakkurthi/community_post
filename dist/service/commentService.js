...
const createComment = async (req, res) => {
    ...
    try {
        const comment = await (0, commentRepository_1.CreateComment)(postId, userId, content);
...
const updateComment = async (req, res) => {
    ...
    try {
        const comment = await (0, commentRepository_1.UpdateComment)(hash, content);
        res.status(201).json({ comment: comment });
    }
    catch (error) {
        res.sendStatus(400);
...
const deleteComment = async (req, res) => {
    ...
    try {
        const deleted = await (0, commentRepository_1.DeleteComments)(hash);
