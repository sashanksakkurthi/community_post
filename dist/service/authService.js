...
const register = async (req, res) => {
    ...
    try {
        const user = await (0, authRepository_1.RegisterData)(firstName, lastName, email, hashPassword);
...
const login = async (req, res) => {
    ...
    try {
        const user = await (0, authRepository_1.LoginData)(email);
        if (!user) {
            res.sendStatus(401);
        }
        else {
...
const verifyUser = async (req, res) => {
    ...
    const user = await (0, authRepository_1.VerifyData)(email);
...
const updateUser = async (req, res) => {
    ...
    try {
        const user = await (0, authRepository_1.UpdateUserData)(hash, firstName, lastName);
        res.status(200).json({ user: user });
    }
    catch (error) {
        res.status(400);
...
const deleteUser = async (req, res) => {
    const hash = req.body.hash;
    try {
        await (0, authRepository_1.DeleteUserData)(hash);
        res.status(200);
    }
    catch (error) {
        res.status(400);
